...
function classifyUpdate(current: string, latest: string): "major" | "minor" | "patch" | "unknown" {
  const cur = current.split(".").map(Number);
  const lat = latest.split(".").map(Number);

  if (cur.length < 3 || lat.length < 3) return "unknown";
  if (isNaN(cur[0]) || isNaN(lat[0])) return "unknown";
...
export async function checkOutdated(packageJsonContent: string): Promise<{
  ...
  for (let i = 0; i < deps.length; i += batchSize) {
    ...
    const latestVersions = await Promise.allSettled(
      batch.map((dep) => getLatestVersion(dep.name))
    );
  ...
  const formatSection = (title: string, items: typeof results) => {
    if (items.length === 0) return "";
    let section = `## ${title} (${items.length})\n\n`;
    for (const item of items) {
      section += `- **${item.name}**: ${item.current} -> ${item.latest} (${item.group})`;
      if (item.error) section += ` [error: ${item.error}]`;
      section += "\n";
    }
    section += "\n";
    return section;
