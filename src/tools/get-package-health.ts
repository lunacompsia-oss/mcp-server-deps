...
export async function getPackageHealth(
  ...
  if (ghRepo) {
    const ghInfo = await fetchJSONSafe<GitHubRepoInfo>(`https://api.github.com/repos/${ghRepo}`);
    ...
    if (ghInfo) {
      ...
      if (ghInfo.archived) output += `- **Status**: ARCHIVED\n`;
    } else {
  ...
  if (times) {
    ...
    if (lastPublished) {
      ...
      if (daysSince > 365) signals.push("WARNING: Not published in over a year");
      else if (daysSince > 180) signals.push("NOTE: Not published in over 6 months");
      else signals.push("OK: Recently maintained");
    }
  }

  if (maintainers.length === 1) signals.push("NOTE: Single maintainer (bus factor risk)");
  else if (maintainers.length >= 3) signals.push("OK: Multiple maintainers");

  if (downloads !== null) {
    if (downloads > 1_000_000) signals.push("OK: Very high download count (widely used)");
    else if (downloads > 100_000) signals.push("OK: High download count");
    else if (downloads < 100) signals.push("WARNING: Very low download count");
  }
