...
async function buildTree(
  ...
  const node: TreeNode = { name: packageName, version, dependencies: [] };
...
function formatTree(node: TreeNode, prefix: string = "", isLast: boolean = true): string {
  ...
  if (node.error === "circular") line += " (circular)";
  else if (node.error) line += ` (error: ${node.error})`;
  line += "\n";
...
function countDeps(node: TreeNode): number {
