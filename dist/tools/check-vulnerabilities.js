 ...
 */
import { queryVulnerabilities } from "../lib/osv.js";
function formatSeverity(vuln) {
    if (!vuln.severity || vuln.severity.length === 0)
        return "Unknown";
    return vuln.severity.map((s) => `${s.type}: ${s.score}`).join(", ");
}
function formatAffectedRange(vuln) {
...
function formatReferences(vuln) {
...
export async function checkVulnerabilities(packageName, version) {
    ...
    for (const vuln of vulns) {
        ...
        if (vuln.published)
            output += `**Published**: ${vuln.published}\n`;
        if (vuln.modified)
            output += `**Modified**: ${vuln.modified}\n`;
...
//# sourceMappingURL=check-vulnerabilities.js.map