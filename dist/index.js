...
server.tool("check_outdated", "Check for outdated dependencies in a project. Parses package.json content and compares each dependency against the latest version on npm. Returns which packages are outdated with current vs latest versions and update type (major/minor/patch).", {
    package_json: z
        .string()
...
server.tool("check_vulnerabilities", "Check a specific package for known security vulnerabilities using the OSV.dev database. Returns vulnerability IDs, severity, affected versions, and fix information.", {
    package_name: z
        .string()
...
server.tool("get_dependency_tree", "Get the dependency tree for a specific npm package. Recursively resolves what it depends on, displayed as an ASCII tree. Useful for understanding transitive dependencies and package weight.", {
    package_name: z
        .string()
...
server.tool("get_package_health", "Get health and quality metrics for an npm package: last publish date, weekly downloads, maintainer count, version history, and GitHub stats (stars, issues, forks). Includes a health summary with warnings.", {
    package_name: z
        .string()
